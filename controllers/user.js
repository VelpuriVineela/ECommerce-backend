...
const userRegistration = async (req, res) => {
  ...
  res.json({
    success: true,
    message: "User successfuly registered, please login to continue",
  });
};
...
const userLogin = async (req, res) => {
  ...
  const expiryDateTime = Math.floor(new Date().getTime() / 1000) + 7200; //1hr from now
...
const saveUserAddress = async (req, res) => {
  ...
  res.json({
    success: true,
    message: "Dummt Save address API",
  });
};
